"""Minimal Policy subclass for PiSerfBehavior models that handles tuple unpacking.
...
from serf_b1k.models.observation import Observation  # Use our custom Observation with FAST fields

class PiSerfBehaviorPolicy(Policy):
    """Policy for PiSerfBehavior models - only difference is unpacking the tuple return.
    
    PiSerfBehavior.sample_actions() returns (actions, subtask_logits) instead of just actions.
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None, initial_actions: np.ndarray | None = None) -> dict:
        """Infer with PiSerfBehavior-specific tuple unpacking.
        ...
        # ONLY DIFFERENCE: Unpack tuple return from PiSerfBehavior.sample_actions

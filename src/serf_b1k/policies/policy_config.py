...
from serf_b1k.policies.pi_serf_behavior_policy import PiSerfBehaviorPolicy
...
def create_trained_policy(
    ...
    *,
    ...
    # Load correlation matrix for PiSerfBehavior models
    if isinstance(model, PiSerfBehavior):
        if norm_stats is None:
            raise ValueError("PiSerfBehavior requires norm_stats but none found.")
    ...
    # For PiSerfBehavior models during inference, skip training-specific transforms
    ...
    # Use custom PiSerfBehaviorPolicy for PiSerfBehavior models (handles tuple unpacking)
    if isinstance(model, PiSerfBehavior):
        return PiSerfBehaviorPolicy(
            model,

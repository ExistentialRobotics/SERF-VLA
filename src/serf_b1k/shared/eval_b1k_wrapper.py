...
from serf_b1k.policies.b1k_policy import extract_state_from_proprio
from serf_b1k.models.pi_serf_behavior_config import TASK_NUM_STAGES
...
class B1KPolicyWrapper():
    """B1K policy wrapper for PiSerfBehavior models with action compression, rolling inpainting, and stage voting."""
    
    def __init__(
        self, 
        policy: BasePolicy,
        text_prompt: str = "PiSerfBehavior model (task-conditioned)",  # Not used, kept for compatibility
        ...
        # PiSerfBehavior specific (always True for B1K)
    ...
    def process_obs(self, obs: dict) -> dict:
        ...
        if "observation/points/xyz" in obs:
            processed_obs["observation/points/xyz"] = obs["observation/points/xyz"]
        if "observation/points/rgb" in obs:
            processed_obs["observation/points/rgb"] = obs["observation/points/rgb"]
        if "observation/points/feat" in obs:
            processed_obs["observation/points/feat"] = obs["observation/points/feat"]
        if "observation/points/robot_mask" in obs:
            processed_obs["observation/points/robot_mask"] = obs["observation/points/robot_mask"]
        # Always provide normalization stats (required by PT Local, harmless for others)
        if "observation/points/pc_norm_offset" in obs:
            processed_obs["observation/points/pc_norm_offset"] = obs["observation/points/pc_norm_offset"]
            processed_obs["observation/points/pc_norm_scale"] = obs["observation/points/pc_norm_scale"]
        elif self.task_id is not None:
            from serf_b1k.utils import get_pc_norm_params, point_cloud_stats
            task_key = f"task-{self.task_id:>04}"
            if task_key in point_cloud_stats:
                offset, scale = get_pc_norm_params(task_key)
                processed_obs["observation/points/pc_norm_offset"] = offset
                processed_obs["observation/points/pc_norm_scale"] = np.array([scale], dtype=np.float32)
            else:
                logger.warning(
                    f"point_cloud_stats missing for {task_key}. "
                    f"PT Local requires normalization stats. "
                    f"Add stats to serf_b1k/datas/utils.py:point_cloud_stats."
                )
    ...
    def prepare_batch_for_pi_serf_behavior(self, batch):
        """Prepare batch for PiSerfBehavior model by adding task_id and current_stage."""
    ...
    def act(self, obs: dict) -> torch.Tensor:
        ...
        if self.last_actions is None or self.action_index >= self.config.execute_in_n_steps:
            ...
            model_input = self.prepare_batch_for_pi_serf_behavior(model_input)

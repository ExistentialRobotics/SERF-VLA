...
class DataConfig:
    ...
    # Task ids to specify which tasks to load from the dataset (if None, all tasks will be loaded).
...
class TrainConfig:
    ...
    def assets_dirs(self) -> pathlib.Path:
        ...
        return pathlib.Path(self.assets_base_dir)
...
_CONFIGS = [
    # Baseline configuration with FAST auxiliary training for BEHAVIOR-1K (no LoRA, full fine-tuning)
    TrainConfig(
    ...
    # LoRA configuration for BEHAVIOR-1K (with no FAST auxiliary training)
    TrainConfig(
        name="pi_behavior_b1k_fast--50t_lora",
        exp_name="openpi",  # For wandb logging
        project_name="B1K", # For wandb logging
        model=pi_behavior_config.PiBehaviorConfig(
            paligemma_variant="gemma_2b",            # Freeze the entire Paligemma backbone (no LoRA)
            action_expert_variant="gemma_300m_lora", # Use LoRA for action expert only (keep VLM backbone frozen)
            action_horizon=30,              # Default action horizon
            action_dim=32,                  # Default action dimension
            use_correlated_noise=True,      # Default use_correlated_noise
            correlation_beta=0.5,           # Default correlation beta
            use_fast_auxiliary=False,       # Disable FAST auxiliary training for LoRA config
            fast_loss_weight=0.0,           # No FAST loss for LoRA config
            fast_encoded_dims="0:6,7:23",   # Default FAST encoded dims (if FAST is enabled)
            fast_vocab_size=1024,           # Default FAST vocab size (if FAST is enabled)
            max_fast_tokens=200,            # Default max FAST tokens (if FAST is enabled)
            use_kv_transform=True,          # Default use_kv_transform
            use_knowledge_insulation=False, # Knowledge insulation must be disabled
            subtask_loss_weight=0.0,        # Disable subtask loss for LoRA config to focus on main flow loss
            freeze_vision_backbone=True,    # Default to freezing vision backbone, can be unfrozen if needed
        ),
        freeze_filter=pi_behavior_config.PiBehaviorConfig( 
            # WARNING: Must be consistent with the model config to ensure correct freezing of parameters
        ...
        data=LeRobotB1KDataConfig(
            ...
            use_delta_joint_actions=True, # Defualt to using delta joint actions
            use_fast_tokenization=True,   # Enable FAST tokenization in data pipeline
        ),
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=1_000,
        ...
        num_workers=4,
        batch_size=16,
        ema_decay=None, # EMA disabled for LoRA training
        num_flow_samples=15, # Defualt to using 15 flow samples for training stability
        weight_loader=weight_loaders.PiBehaviorWeightLoader(
            # Checkpoint pre-trained on 50 tasks from BEHAVIOR-1K

...
# Demo torso envelopes derived from expert demonstrations.
# Clamp eval targets to these task-specific absolute joint ranges.
# TASK_TORSO_DEMO_LIMITS = {}

TASK_TORSO_DEMO_LIMITS = {
    21: {
        "min": np.array([0.0206, -2.7925, -1.8326, -0.0], dtype=np.float32),
        "max": np.array([1.8326, 0.3786, 1.4261, 0.0], dtype=np.float32),
    },
    22: {
        "min": np.array([0.4189, -2.7925, -1.8326, -0.0], dtype=np.float32),
        "max": np.array([1.8326, -0.3532, -0.0123, 0.0], dtype=np.float32),
    },
    26: {
        "min": np.array([0.0417, -2.7925, -1.8326, -0.0], dtype=np.float32),
        "max": np.array([1.8326, 0.3433, 0.5933, 0.0], dtype=np.float32),
    },
}
...
def expert_demo_torso_range_correction(
    task_id: int,
    stage: int,
    state: np.ndarray,
    actions: np.ndarray
    ...
    """
    Clamp torso action targets to the expert-demo min/max envelope for known tasks.

    Actions are absolute position targets for the torso on R1 Pro, so staying inside
    the demonstrated range is a simple way to avoid out-of-support postures at eval time.
    """
    del state  # Unused for this task-conditioned clamp.

    limits = TASK_TORSO_DEMO_LIMITS.get(task_id)
    if limits is None:
        return None

    torso_actions = actions[:, TORSO_START_IDX:TORSO_END_IDX]
    clipped_torso = np.clip(torso_actions, limits["min"], limits["max"])

    if np.allclose(clipped_torso, torso_actions, rtol=0.0, atol=1e-6):
        return None

    corrected_actions = actions.copy()
    corrected_actions[:, TORSO_START_IDX:TORSO_END_IDX] = clipped_torso
    logger.info(
        "🔧 TORSO ENVELOPE CORRECTION: Task %s torso actions clamped to demo min/max",
        task_id,
    )
    return corrected_actions, stage
...
GENERAL_RULES = [
    ...
    expert_demo_torso_range_correction,  # Keep torso inside demo support after other corrections
]
...
def apply_correction_rules(
    ...
    2. General rules (e.g., gripper correction policies, torso envelope clamp)
    
    Later rules see the outputs of earlier ones, so independent corrections can
    compose within a single eval step.
    ...
    for rule in task_specific_rules:
        result = rule(task_id, corrected_stage, state, corrected_actions)

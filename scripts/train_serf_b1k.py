...
from serf_b1k.training import config as _config
from serf_b1k.training import data_loader as _data_loader
from serf_b1k.training import weight_loaders as _weight_loaders
...
def _load_weights_and_validate(loader: _weight_loaders.WeightLoader, params_shape: at.Params) -> at.Params:
    ...
    # These fields are either nnx.Intermediate state that is not restored from checkpoints or
    # PiSerf-specific modules intentionally initialized from scratch when loading Pi05 weights.
    excluded_field_names = (
        ...
        "lora",  # LoRA fields may be absent or intentionally skipped
        "point_tokenizer",  # Point tokenizer intermediate fields
        "map_tokenizer",  # New PiSerf module initialized from scratch for Pi05 checkpoints
    )
    ...
    def filter_intermediate_fields(params_dict):
        ...
        filtered = {k: v for k, v in flat.items() 
                   if not any(field in str(k) for field in excluded_field_names)}
    ...
    def should_exclude(k, v):
        if isinstance(v, jax.ShapeDtypeStruct):
            return True
        return any(field in str(k) for field in excluded_field_names)
...
def init_train_state(
    ...
    *, 
    ...
    def init(rng: at.KeyArrayLike, partial_params: at.Params | None = None) -> training_utils.TrainState:
        ...
        if isinstance(model, PiSerfBehavior) and norm_stats is not None:
            model.load_correlation_matrix(norm_stats)
            logging.info("Loaded correlation matrix during model initialization for 3D PCD model")
    ...
    if isinstance(model, PiSerfBehavior) and hasattr(model, 'kv_transform') and model.kv_transform is not None:
...
def train_step(
    ...
    @at.typecheck  
    def loss_fn(
        model: PiSerfBehavior, rng: at.KeyArrayLike, observation: Observation, actions: _model.Actions
    ):
...
def print_trainable_and_frozen_params(params: at.Params, trainable_filter: nnx.filterlib.Filter):
    flat_params = traverse_util.flatten_dict(params.to_pure_dict())

    trainable_params = params.filter(trainable_filter).to_pure_dict()
    flat_trainable_params = traverse_util.flatten_dict(trainable_params, sep="/")

    frozen_params = params.filter(nnx.Not(trainable_filter)).to_pure_dict()
    flat_frozen_params = traverse_util.flatten_dict(frozen_params, sep="/")
    
    logging.info("Trainable Parameters:")
    for i, name in enumerate(flat_trainable_params.keys()):
        logging.info(f"  {i}: {name}")
    
    logging.info("Frozen Parameters:")
    for i, name in enumerate(flat_frozen_params.keys()):
        logging.info(f"  {i}: {name}")
...
def main(config: _config.TrainConfig):
    ...
    print_trainable_and_frozen_params(train_state.params, config.trainable_filter)
    ...
    for step in pbar:
        ...
        if step % config.log_interval == 0:
            ...
            info_str = "  /  ".join(f"{k}={float(v):.4f}" for k, v in main_metrics.items())

...
#   bash scripts/test/test_4d_env_robot_feat_map.sh --task-id 0026 --checkpoint-path exps/path/to/checkpoint/20000
#   bash scripts/test/test_4d_env_robot_feat_map.sh --write-tracking-video true --write-video false -- eval_instance_ids='[0,1,2]'

CONDA_ENV_PYTHON_PATH="/mnt/hdd/miniconda3/envs/behavior/bin/python"

TASK_ID="0021"
CONFIG_PREFIX="pi_serf_behavior_b1k_fast--4d_env_robot_feat_map--50t_lora"
LOGGING_TAG="bs_16_iter_20000_eval"
CHECKPOINT_PATH="exps/pi_serf_behavior_b1k_fast--4d_env_robot_feat_map--50t_lora--task-0021/bs_16--iter_20000/latest"
...
WRITE_TRACKING_VIDEO=false
SERVER_LOG="logs/_server/serve_4d_env_robot_feat_map_b1k.log"
EVAL_ARGS=()
...
Options:
  ...
  --robot-model-root-path PATH    Root directory containing robot map assets.
  ...
  --write-tracking-video BOOL     Enable or disable tracker visualization videos.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --map-dataset-root-path)
            ...
            shift 2
            ;;
        --robot-model-root-path)
            ROBOT_MODEL_ROOT_PATH="$2"
            shift 2
            ;;
        --logging-tag)
            LOGGING_TAG="$2"
...
set +e
"${CONDA_ENV_PYTHON_PATH}" BEHAVIOR-1K/OmniGibson/omnigibson/learning/eval_4d_env_robot_feat_serf_b1k.py \
    ...
    write_tracking_video="${WRITE_TRACKING_VIDEO}" \

...
from serf_b1k.policies import policy_config as _policy_config
from serf_b1k.policies.checkpoint_switcher import CheckpointSwitcher
from serf_b1k.shared.eval_b1k_wrapper import B1KPolicyWrapper, B1KWrapperConfig
...
def create_policy(args: Args) -> _policy.Policy:
    ...
    return _policy_config.create_trained_policy(
        ...
        sample_kwargs=build_sample_kwargs(args),
    )


def build_sample_kwargs(args: Args) -> dict[str, float | int]:
    return {
...
def build_wrapper_config(args: Args) -> B1KWrapperConfig:
    return B1KWrapperConfig(
...
def main(args: Args) -> None:
    ...
    if args.task_id is not None:
        ...
        logging.info("Using PI_BEHAVIOR model - task_id will be extracted from observations")
        task_id = None
    ...
    if args.task_checkpoint_mapping:
        ...
        try:
            ...
            logging.info(
                "Checkpoint switcher initialized - will switch checkpoints based on task_id"
            )
    ...
    wrapper_config = build_wrapper_config(args)
    logging.info(
        "Wrapper config: execute=%s, keep=%s, steps=%s, num_steps=%s",
    ...
    if checkpoint_switcher:
        logging.info(
            "Multi-checkpoint mode: checkpoints will switch based on task_id from observations"
        )
    else:
